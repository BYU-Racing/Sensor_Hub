...
#define FLOWSENSOR_HZ_PER_LPM 7.5     // Calibration: pulses per second (Hz) produced by 1 L/min of flow. 7.5 is from the YF-S201 datasheet.
...
#define FLOWSENSOR_WINDOW_MS 1000     // In milliseconds: how long to count pulses before calculating a new flow rate.
                                      // Longer = smoother reading but slower to update.



void flowSensorInit(uint8_t pin);  // Call once in setup(). Sets up the pin and starts counting pulses.
void flowSensor_isr();             // Interrupt handler: runs automatically on every pulse. Do not call directly.
float flowSensor_rate_lpm();       // Returns flow rate in liters per minute. Safe to call as often as you like.
